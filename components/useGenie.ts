...
import { matchForge } from "./forgeBlueprints";
import { tryShowBuyPrompt } from "./buyPromptGate";
export function useGenie() {
  const [ready, setReady] = useState(false);
  const [S, setS] = useState<State>(defaultState);
  const [msg, setMsg] = useState("");
  const [buyNudge, setBuyNudge] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    state = granted.state;
    if (granted.granted) note = note || "Owner lamp: +500,000 TC.";
  ...
  const softNeedCoins = (need: number) => {
    setMsg("Need " + need + " TC.");
    // Occasional quiet Shop nudge — never auto-navigate; gate throttles modals.
    if (tryShowBuyPrompt()) {
      setBuyNudge(
        "Lamp's light on low coin. Quiet path: Shop packs, or a Process pack under Tools."
      );
    }
  };
  ...
  const claimLamp = (rawEmail: string) => {
    ...
    persist(next);
    if (granted.granted) {
      const note = "Owner lamp: +500,000 TC.";
      setPayNote(note);
      setMsg(note);
  ...
  const addFreeTool = (freeId: string, name: string) => {
    ...
    const meta = getFreeTool(freeId);
    persist({
      ...cur,
      tools: [
        {
          n: name,
          r: meta?.rarity || "common",
          t: Date.now(),
          freeId,
        },
        ...cur.tools,
  ...
  const buyProcessPack = (packId: string) => {
    ...
    const cur = stateRef.current;
    const owned = cur.ownedPacks || [];
    if (owned.includes(pack.id)) {
      setMsg("Already own this process pack.");
      return;
    }
    if (!spend(pack.cost)) {
      softNeedCoins(pack.cost);
      return;
    }
    const afterSpend = stateRef.current;
    const additions = [];
    for (const tid of pack.toolIds) {
      const meta = getFreeTool(tid);
      if (!meta) continue;
      if (afterSpend.tools.some((t) => t.freeId === tid)) continue;
      additions.push({
        n: meta.n,
        r: meta.rarity,
        t: Date.now(),
    ...
    persist({
      ...afterSpend,
      ownedPacks: [...owned, pack.id],
      tools: [...additions, ...afterSpend.tools],
    });
    setBuyNudge(null);
    setMsg("Packed · " + pack.n + " · " + pack.toolIds.length + " tools unlocked.");
    go("tools");
  };
  const dismissBuyNudge = () => setBuyNudge(null);
  ...
  const onForge = () => {
    ...
    setNeed("");
    try {
      sessionStorage.setItem("tg_open_latest", "1");
    } catch {}
    go("tools");
    setMsg("Forged " + bp.name + ". Open it and tap Try it.");
  };
